...
class aspnet_bin_exposure(BaseModule):
    ...
    in_scope_only = True
    test_dlls = ["Telerik.Web.UI.dll", "Newtonsoft.Json.dll", "System.Net.Http.dll", "EntityFramework.dll", "AjaxControlToolkit.dll"]
    ...
    async def handle_event(self, event):
        ...
        for test_dll in self.test_dlls:
            for technique in ["b/(S(X))in/###DLL_PLACEHOLDER###/(S(X))/","(S(X))/b/(S(X))in/###DLL_PLACEHOLDER###"]:
                ...
                if test_result:
                    if test_result.status_code == 200 and ('content-type' in test_result.headers and 'application/x-msdownload' in test_result.headers['content-type']):
                        self.debug(f"Got positive result for probe with test url: [{test_url}]. Status Code: [{test_result.status_code}] Content Length: [{len(test_result.content)}]")
                        description = f"IIS Bin Directory DLL Exposure. Detection Url: [{test_url}]"
                        await self.emit_event(
                            {"severity": "HIGH", "host": str(event.host), "url": normalized_url, "description": description},
                            "VULNERABILITY",
                            event,

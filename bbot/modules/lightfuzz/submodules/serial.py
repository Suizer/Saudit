            ...
            for type, payload in payload_set.items():
                ...
                if (
                    ...
                    self.results.append(
                        {
                            "type": "FINDING",
                            "description": f"POSSIBLE Unsafe Deserialization. {self.metadata()} Technique: [Error Resolution (Baseline: [{payload_baseline.baseline.status_code}] {self.lightfuzz.helpers.beautifulsoup(payload_baseline.baseline.text, 'html.parser') and self.lightfuzz.helpers.beautifulsoup(payload_baseline.baseline.text, 'html.parser').title and self.lightfuzz.helpers.beautifulsoup(payload_baseline.baseline.text, 'html.parser').title.string and f"'{self.lightfuzz.helpers.truncate_string(self.lightfuzz.helpers.beautifulsoup(payload_baseline.baseline.text, 'html.parser').title.string, 50)}'" or ''} -> Probe: [{status_code}] {self.lightfuzz.helpers.beautifulsoup(response.text, 'html.parser') and self.lightfuzz.helpers.beautifulsoup(response.text, 'html.parser').title and self.lightfuzz.helpers.beautifulsoup(response.text, 'html.parser').title.string and f"'{self.lightfuzz.helpers.truncate_string(self.lightfuzz.helpers.beautifulsoup(response.text, 'html.parser').title.string, 50)}'" or ''})] Serialization Payload: [{type}]",
                        }
                    )

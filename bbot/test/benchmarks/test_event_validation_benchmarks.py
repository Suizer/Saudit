...
class TestEventValidationBenchmarks:
    ...
    def _generate_diverse_event_data(self, count=1000):
        ...
        for _ in range(count // 10):
            # Standard domains
            event_data.append(f"{rng.choice(subdomains)}.{rng.choice(['example', 'test', 'evilcorp'])}.{rng.choice(tlds)}")
            # Bare domains
        ...
        for _ in range(count // 15):
            # IPv4
            event_data.append(f"{rng.randint(1,254)}.{rng.randint(1,254)}.{rng.randint(1,254)}.{rng.randint(1,254)}")
            # IPv6
    ...
    @pytest.mark.benchmark(group="make_event_dummy_comparison")
    def test_make_event_dummy_vs_full_validation(self, benchmark):
        """Compare performance between dummy=True and dummy=False modes"""
        event_data = self._generate_diverse_event_data(50)  # Smaller batch for dummy=False
        
        def create_events_dummy_mode():
            dummy_events = []
            
            for data in event_data:
                # Use dummy=True for minimal validation
                event = make_event(data, dummy=True)
                dummy_events.append(event)
            
            return {
                'events_created': len(dummy_events),
                'mode': 'dummy',
                'validation_level': 'minimal'
            }
        
        result = benchmark(create_events_dummy_mode)
        assert result['events_created'] == len(event_data)
        assert result['mode'] == 'dummy'

    @pytest.mark.benchmark(group="make_event_type_specific")
    def test_make_event_dns_names_only(self, benchmark):
        """Benchmark make_event specifically with DNS names to test DNS validation logic"""
        # Generate only DNS names to focus on DNS-specific validation
        rng = random.Random(42)
        subdomains = ["www", "api", "mail", "ftp", "admin", "test", "dev", "staging"]
        domains = ["example", "test", "company", "evilcorp"]
        tlds = ["com", "org", "net", "io"]
        ...
        for _ in range(100):
            ...
            tld = rng.choice(tlds)
            dns_names.append(f"{subdomain}.{domain}.{tld}")
        
        def create_dns_events():
            dns_events = []
            
            for dns_name in dns_names:
                event = make_event(dns_name, dummy=True)
                dns_events.append(event)
            
            return {
                'dns_events_created': len(dns_events),
                'all_dns_type': all(event.type == 'DNS_NAME' for event in dns_events)
            }
        
        result = benchmark(create_dns_events)
        assert result['dns_events_created'] == len(dns_names)
        assert result['all_dns_type']  # All should be detected as DNS_NAME type 
...
@pytest.mark.asyncio
async def test_config(saudit_scanner):
    ...
    scan1 = saudit_scanner("127.0.0.1", modules=["ipneighbor"], config=config)
    await scan1.load_modules()
    assert scan1.config.web.user_agent == "SAUDIT Test User-Agent"

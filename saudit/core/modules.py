...
log = logging.getLogger("saudit.module_loader")

saudit_code_dir = Path(__file__).parent.parent
...
class ModuleLoader:
    ...
    default_module_dir = saudit_code_dir / "modules"
            ...
            for module_file in list_files(module_dir, filter=self.file_filter):
                ...
                if preloaded and module_cache_key == cache_key:
                    ...
                    if module_dir.name == "modules":
                        namespace = "saudit.modules"
                    else:
                        namespace = f"saudit.modules.{module_dir.name}"
                    try:

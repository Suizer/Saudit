...
from saudit.core.helpers.misc import make_table, mkdir, get_closest_match


log = logging.getLogger("saudit.presets")
...
class Preset(metaclass=BasePreset):
    ...
    @property
    def saudit_home(self):
        return Path(self.config.get("home", "~/.saudit")).expanduser().resolve()
    ...
    @property
    def preset_dir(self):
        return (self.saudit_home / "presets").expanduser().resolve()
    ...
    def helpers(self):
        if self._helpers is None:
            from saudit.core.helpers.helper import ConfigAwareHelper

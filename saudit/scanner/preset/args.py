...
from saudit.core.helpers.misc import chain_lists, get_closest_match, get_keys_in_dot_syntax

log = logging.getLogger("saudit.presets.args")
    ...
    def create_parser(self, *args, **kwargs):
        ...
        presets.add_argument(
            ...
            help=(
                "Preset(s) to enable.\n"
                "\n"
                "Main presets (progressive — each extends the previous):\n"
                "  initial                   Passive recon: fingerprinting, JS analysis, API discovery,\n"
                "                            port scan, Git exposure, CMS detection (no brute-force)\n"
                "  web-basic                 Adds: ffuf dirbust, 403 bypass, baddns, file harvesting, API probe\n"
                "  web-authenticated         Adds: session check, paramminer, lightfuzz (requires --bearer or -C)\n"
                "  web-authenticated-thorough Adds: host header, SSRF, smuggling, URL bypass (approved targets only)\n"
                "\n"
                "Nuclei sub-presets (add when ready — cms_advisor recommends the right one automatically):\n"
                "  nuclei                    Full template library, directory roots only\n"
                "  nuclei-budget             ~10 req/host — use when WAF detected\n"
                "  nuclei-technology         Templates matching detected technologies only — most surgical\n"
                "  nuclei-intense            All URLs, no directory_only restriction (combine with spider)\n"
                "\n"
                "Directory brute-force:\n"
                "  dirbust-light             Surface-level ffuf (includes IIS shortname enumeration)\n"
                "  ffuf-heavy                Recursive ffuf (depth 3, extensions, 5000-line wordlist)\n"
                "                            For IIS: combine with -p iis-shortnames or -p dotnet-audit\n"
                "  iis-shortnames            IIS 8.3 shortname enumeration\n"
                "  dotnet-audit              Full IIS/.NET audit: Telerik, DotNetNuke, AjaxPro, bin exposure\n"
                "\n"
                "Parameter fuzzing (ai_review recommends the right level based on scan signals):\n"
                "  lightfuzz-xss             XSS via GET only — most surgical\n"
                "  lightfuzz-light           All vuln classes, no POST\n"
                "  lightfuzz-medium          All vuln classes, no POST + paramminer + badsecrets + hunt\n"
                ...
                "  paramminer                Parameter discovery only (no fuzzing)\n"
                "\n"
                "Standalone modules (use -m, not -p — cms_advisor recommends these when relevant):\n"
                "  -m wpscan                 WordPress vuln scan (when WordPress detected)\n"
                "  -m mendix_recon           Mendix access-control testing (when Mendix detected)\n"
                "  -m from_report            Re-seed from a previous scan's output.json\n"
                "\n"
